function validateSentence(sentence) {
    ...
    for (var k = 0; k < sentence.tokens.length; k++) {
        ...
        if ( k + 3 < sentence.tokens.length ) {
            if (
                sentence.tokens[k].surface.match(/[一二三四五六七八九十百千]/) &&
                sentence.tokens[k+1].surface.match(/[\u4E00-\u9FFF]/) &&
                sentence.tokens[k+2].surface.match(/[一二三四五六七八九十百千]/) &&
                sentence.tokens[k+3].surface.match(/[\u4E00-\u9FFF]/)
            ) return null;
        }
        ...
        if ( k + 1 < sentence.tokens.length ){
            if (
                sentence.tokens[k].surface === '数' &&
                sentence.tokens[k+1].surface.match(/[十百千万]/)
            ) return null;
        }
        ...
        if (k + 2 < sentence.tokens.length){
            if (
                sentence.tokens[k].surface.match(new RegExp(/[一二三四五六七八九十百千]/)) &&
                sentence.tokens[k+1].surface === '次' &&
        ...
        if( k + 1 < sentence.tokens.length ){
            if (
                ...
                sentence.tokens[k+1].surface ==='大陸'
            ) return null;
        }
        ...
        if ( sentence.tokens[k].surface.match(/十進法|二進法|十六進法/) ) return addError('数を数えられるものは算用数字を利用します' , sentence );

        // 三つが「三つ」という一般名詞でtokenizeされてしまうので、個別に定義
        if (
             sentence.tokens[k].tags[0] === "名詞" &&
             sentence.tokens[k].tags[1] === "一般" &&
             sentence.tokens[k].surface.match(new RegExp(/[一二三四五六七八九]つ/))
        ){
        ...
        if ( k + 2 < sentence.tokens.length ){
            if (
                sentence.tokens[k].surface.match(new RegExp(/[1-9]/)) &&
                sentence.tokens[k+1].surface === '時' &&
                sentence.tokens[k+2].surface === '的'
            ) addError('慣用的表現、熟語、概数、固有名詞、副詞など、漢数字を使用することが一般的な語句では漢数字を使います。' , sentence );
        }
    }
}
