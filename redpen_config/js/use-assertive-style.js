        ...
        for (var i = 0; i < sentence.tokens.length; i++) {
            ...
            if ( i + terms.tokens.length - 1 < sentence.tokens.length ){
                ...
                for (var j = 0; j < terms.tokens.length; j++){
                    ...
                    if (sentence.tokens[i+j].surface === "わかる" && sentence.tokens[i+j+1].tags[0] === "形容詞"){ result--; }
                    if (sentence.tokens[i+j].surface === "分かる" && sentence.tokens[i+j+1].tags[0] === "形容詞"){ result--; }

                    if (
                        sentence.tokens[i+j].tags[0] === terms.tokens[j].tags0 &&
                        sentence.tokens[i+j].tags[1] === terms.tokens[j].tags1 &&
                        sentence.tokens[i+j].surface === terms.tokens[j].tags6
                    ){
                        result++;
                    }
                }
